    ...
    pool.on('share', function(isValidShare, isValidBlock, data) {
        ...
        if (isValidShare) {
            ...
            if (config.printHighShares) {
                ...
                switch (true) {
                    case (sillyPercent > 100):
                        logging('PoolWorker', 'error', 'Share was found with diff higher than 100%! (' + sillyPercent.toFixed(0) + '%) ' + sdiff);
                        break;
                    case (sillyPercent > 75):
                        logging('PoolWorker', 'special', 'Share was found with diff higher than 75%! (' + sillyPercent.toFixed(0) + '%) ' + sdiff);
                        break;
                    case (sillyPercent > 50):
                        logging('PoolWorker', 'special', 'Share was found with diff higher than 50%! (' + sillyPercent.toFixed(0) + '%) ' + sdiff);
                        break;
                }
            }
            if (config.printShares) {
                ...
                if (data.blockDiffActual > data.shareDiff) {
                    logging('PoolWorker', 'debug', 'Share accepted - Block diff: ' + data.blockDiffActual + ' Share Diff: ' + data.shareDiff + ' (' + sillyPercent.toFixed(2) + '%) '+ data.worker + ' ' + shareCount[data.worker] + ' shares');
                } else {
